...
public class JeuMultiModel extends ModeleAnimationDevint {
    ...
    public JeuMultiModel() {
    	mouvementJoueur1= new boolean[CstJeuMulti.SENS.values().length];
    	mouvementJoueur2= new boolean[CstJeuMulti.SENS.values().length];
    }
    ...
    private void bouge(Positions joueur,boolean[] notification){
    	if (notification[CstJeuMulti.SENS.bas.ordinal()])
    		joueur.moveBot();
    	if (notification[CstJeuMulti.SENS.haut.ordinal()])
    		joueur.moveTop();
    	if (notification[CstJeuMulti.SENS.gauche.ordinal()])
    		joueur.moveLeft();
    	if (notification[CstJeuMulti.SENS.droite.ordinal()])
    		joueur.moveRight();
    }
